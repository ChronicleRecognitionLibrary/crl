...
namespace CRL 
{
  ...
  */  
  ChronicleDuring::ChronicleDuring(Chronicle* opL, Chronicle* opR) 
    : ChronicleBinaryOp(opL,opR) 
  {
    // Is the chronicle is used, it is not "purgeable" anymore
    opL->setPurgeable(false);          

    // Ce(C1 during C2) = Cr(C1) U Cr(C2) \ {\bot}
    // Cr(C1 during C2) = Ce(C1 during C2) U {\bot}
    ...
    if ( Context::contextIntersection(opLCr, opRCr).isSingletonAnonymous() )
    {
      ...
      throw("ChronicleDuring : inappropriate contexts");
  }
  ...
  *   \return string
  */
  std::string ChronicleDuring::toString() const 
  {
  ...
  */
  bool ChronicleDuring::process(const DateType& d, CRL::Event* e)
  {
             ...
             itL != _opLeft->getRecognitionSet().end(); itL++)
        {
          ...
          if ( applyPredicate(x1x2) )
          {
            if (  ((*itL)->getMinOrder() > (*itR)->getMinOrder())
              && ((*itL)->getMaxOrder() < (*itR)->getMaxOrder())  )
            {
