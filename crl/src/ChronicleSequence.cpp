...
namespace CRL 
{
  ...
  */  
  ChronicleSequence::ChronicleSequence(Chronicle* opL, Chronicle* opR) 
    : ChronicleBinaryOp(opL,opR) 
  {
    // If the chronicle is used in a SEQ, it is not "purgeable" anymore
    ...
    if ( Context::contextIntersection(opLCr, opRCr).isSingletonAnonymous() )
    {
      ...
      throw("ChronicleSequence : inappropriate contexts");
  }
  ...
  *   \return string
  */
  std::string ChronicleSequence::toString() const 
  {
  ...
  */
  bool ChronicleSequence::process(const DateType& d, CRL::Event* e) 
  {
