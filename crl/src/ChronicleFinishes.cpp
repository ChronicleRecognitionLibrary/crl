...
namespace CRL
{
  ...
  */  
  ChronicleFinishes::ChronicleFinishes(Chronicle* opL, Chronicle* opR) 
    : ChronicleBinaryOp(opL,opR) 
  {
    // Ce(C1 finishes C2) = Cr(C1) U Cr(C2) \ {\bot}
    // Cr(C1 finishes C2) = Ce(C1 finishes C2) U {\bot}
    ...
    if ( Context::contextIntersection(opLCr, opRCr).isSingletonAnonymous() )
    {
      ...
      throw("ChronicleFinishes : inappropriate contexts");
  }
  ...
  *   \return string
  */
  std::string ChronicleFinishes::toString() const
  {
  ...
  */
  bool ChronicleFinishes::process(const DateType& d, CRL::Event* e)
  {
