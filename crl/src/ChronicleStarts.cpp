/** ***********************************************************************************
 * \file ChronicleStarts.cpp
 * \author Ariane Piel & Jean Bourrely / Onera DCPS
 * \date 2012
 * \brief One chronicle starting another chronicle
 **************************************************************************************/
...
namespace CRL 
{
  ...
  */  
  ChronicleStarts::ChronicleStarts(Chronicle* opL, Chronicle* opR) 
    : ChronicleBinaryOp(opL,opR) 
  {
    // If the chronicle is used, it is not "purgeable" anymore
    opL->setPurgeable(false);   

     // Ce(C1 starts C2) = Cr(C1) U Cr(C2) \ {\bot}
    // Cr(C1 starts C2) = Ce(C1 starts C2) U {\bot}
    ...
    if ( Context::contextIntersection(opLCr, opRCr).isSingletonAnonymous() )
    {
      ...
      throw("ChronicleStarts : inappropriate contexts");
  }
  ...
  *   \return string
  */
  std::string ChronicleStarts::toString() const
  {
  ...
  */
  bool ChronicleStarts::process(const DateType& d, CRL::Event* e)
  {
