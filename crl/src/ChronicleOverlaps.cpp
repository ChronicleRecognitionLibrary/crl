...
namespace CRL 
{
  ...
  */  
  ChronicleOverlaps::ChronicleOverlaps(Chronicle* opL, Chronicle* opR)
    : ChronicleBinaryOp(opL,opR) 
  {
    // If the chronicle is used, it is not "purgeable" anymore
    opL->setPurgeable(false);  

    // Ce(C1 overlaps C2) = Cr(C1) U Cr(C2) \ {\bot}
    // Cr(C1 overlaps C2) = Ce(C1 overlaps C2) U {\bot}
    ...
    if ( Context::contextIntersection(opLCr, opRCr).isSingletonAnonymous() )
    {
      ...
      throw("ChronicleOverlaps : inappropriate contexts");
  }
  ...
  *   \return string
  */
  std::string ChronicleOverlaps::toString() const
  {
  ...
  */
  bool ChronicleOverlaps::process(const DateType& d, CRL::Event* e)
  {
           ...
           itR != _opRight->getNewRecognitions().end(); itR++)
      {
             ...
             itL != _opLeft->getRecognitionSet().end(); itL++)
        {
          if (   ((*itL)->getMinOrder() < (*itR)->getMinOrder())
              && ((*itR)->getMinOrder() < (*itL)->getMaxOrder())
